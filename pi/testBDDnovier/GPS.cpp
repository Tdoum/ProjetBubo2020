...
int  GPS::lire () 
{
	...
	/* Effacement de la salve*/
	salve.erase(salve.begin(),salve.end());
	do
	{
		...
		if ((character[0]>=0x24) && (character[0]<=0x5A)) //5A pour majuscules
		{
			salve.push_back(character[0]);
		}
		else if ((character[0]>=0x61) && (character[0]<=0x7A)) 
		{
			salve.push_back(character[0]-0x20);
		}
	}
	while (salve.size()<400);
	close(fd);
	return retour;
}

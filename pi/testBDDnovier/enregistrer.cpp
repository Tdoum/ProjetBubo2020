...
{       Logger* monLogger = new Logger();
	...
	cout<<HTTPHTMLHeader();
			...
			//--------------------------Rï¿½cupï¿½ration altitude -------------------------
                        fvalue = cgi.getElement("altitude"); 
                        if( !fvalue->isEmpty() && fvalue != (*cgi).end()) { 
                                                                fichier << "L'altitude est  : " << **fvalue << endl; 
                                aAltitude = atof((**fvalue).c_str()); // atof => convertit un string en double 
                        } 
                        else 
			...
			Enregistrement* enregistrement = new Enregistrement(albedo, fluxLum, aIdRaspberry,  lamp,  streetlight,height, aLatitude,  aLongitude, aAltitude, aNbImages, town,  street,  ULOR, aDate, aNotes, power);
